...
import {
  ...
  ALL_RESTAURANT_DAY_WISE_OPTIONS,
  ALL_RESTAURANT_DAY_WISE_ROWS,
  summarizeAllRestaurantDayWise,
  type AllRestaurantDayWiseColumnKey,
  type AllRestaurantDayWiseRow,
} from '../../mocks/allRestaurantDayWiseData'
...
function ColumnsMenu({
  ...
  visible: Record<AllRestaurantDayWiseColumnKey, boolean>
  onToggle: (key: AllRestaurantDayWiseColumnKey) => void
}) {
    ...
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <ul className="absolute left-0 z-40 mt-1.5 max-h-72 w-64 overflow-y-auto rounded-lg border border-line bg-card py-1 shadow-lg">
          {ALL_RESTAURANT_DAY_WISE_COLUMNS.map((column) => (
            <li key={column.key}>
...
function summaryCell(
  ...
  if (!stats) return '—'
  if (key === 'restaurant' || key === 'date' || key === 'invoiceNos') return ''
  return formatAmount(stats[key] ?? 0)
}

export default function AllRestaurantDayWiseReport() {
  ...
  const [visible, setVisible] = useState<
    Record<AllRestaurantDayWiseColumnKey, boolean>
  >(() =>
    Object.fromEntries(
      ALL_RESTAURANT_DAY_WISE_COLUMNS.map((column) => [column.key, true]),
    ) as Record<AllRestaurantDayWiseColumnKey, boolean>,
  )

  const columns = useMemo(
    () =>
      ALL_RESTAURANT_DAY_WISE_COLUMNS.filter((column) => visible[column.key]),
    [visible],
  )

  const rows = useMemo(() => {
    if (!appliedRestaurant) return ALL_RESTAURANT_DAY_WISE_ROWS
    return ALL_RESTAURANT_DAY_WISE_ROWS.filter(
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-line bg-card p-3">
        ...
        <div className="min-w-[220px] flex-1">
          <SearchableSelect
            label="Restaurants"
            value={restaurant}
            options={ALL_RESTAURANT_DAY_WISE_OPTIONS}
            ...
            <tbody>
              ...
              {rows.length === 0 ? (
                <tr>
                  ...
                  >
                    No day-wise sales found for the selected filters.
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
