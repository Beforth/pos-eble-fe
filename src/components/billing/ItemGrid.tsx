...
interface ItemGridProps {
  ...
  /** Show animated pink heart on cards (Favorite Items section). */
  showFavoriteHeart?: boolean
}

function stripeClass(tags: string[], name: string) {
  ...
  if (isEgg) return 'bg-secondary'
  if (isNonVeg) return 'bg-primary'
  return 'bg-success'
}
            ...
            {items.map((item, index) => (
              ...
              >
                <button
                  ...
                  className={`group relative flex h-[72px] w-full items-center overflow-hidden rounded-xl border border-line bg-card pl-2.5 pr-3 text-left shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-all duration-300 ease-out will-change-transform disabled:cursor-not-allowed disabled:opacity-45 ${
                    item.available
                ...
                >
                  <span
                    className={`absolute inset-y-0 left-0 w-1 transition-all duration-300 group-hover:w-1.5 ${stripeClass(item.tags, item.name)}`}
                  />
                  <span className="billing-item-shine" aria-hidden />
                  <span className="relative min-w-0 flex-1 pl-1.5 pr-5">
